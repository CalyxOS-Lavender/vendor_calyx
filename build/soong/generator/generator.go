...
func init() {
	android.RegisterModuleType("calyx_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = calyxExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = calyxExpandVariables(ctx, dep_file)
	...
	cmd := calyxExpandVariables(ctx, String(g.properties.Cmd))
